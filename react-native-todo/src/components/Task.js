...
const Task = ({item,deleteTask,toggleTask,updateTask}) => {
    ...
    return isEditing ? (
        ...
        <Container>
            ...
            <IconButton type={item.completed === true ? images.complete : images.uncomplete} id={item.id} onPressOut={toggleTask}/>
            <Contents completed={item.completed}>{item.text}</Contents>
            {/* 완료상태면 아이콘이 안보이게 수정하기 */}
            {item.completed || <IconButton type={images.update} onPressOut={_handleUpdatebuttonPress}/>}
            {/* {<IconButton type={item.completed === true ? '' : images.update}/>} */}
